...
# Wait until the launch time to launch the EC2 instance
...
# Wait until the termination time to terminate the EC2 instance
...
# Update the .env file with the given key and value (The instance ID of the EC2 Instance)
...
# Get the launch and termination times for the EC2 instance
def get_launch_and_termination_times():
    ...
    while True:
        try:
            ...
            print()
            now = datetime.now()
            launch_time = datetime.strptime(launch_time_str, "%H:%M:%S").replace(year=now.year, month=now.month, day=now.day, microsecond=0)
            termination_time = datetime.strptime(termination_time_str, "%H:%M:%S").replace(year=now.year, month=now.month, day=now.day, microsecond=0)
            if launch_time < now:
